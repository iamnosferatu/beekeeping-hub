// frontend/src/components/articles/ArticleListEmpty.js
 ...
 */
const ArticleListEmpty = ({ search, tag }) => {
  /**
   ...
   * @returns {string} Appropriate title for the empty state
   */
  const getTitle = () => {
    if (search && tag) {
   ...
   * @returns {string} Helpful message for the user
   */
  const getMessage = () => {
    if (search && tag) {
